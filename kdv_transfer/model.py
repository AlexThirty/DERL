...
class KdVPINN(torch.nn.Module):
    ...
    def eval_losses(self, step,
                ...
                y_distill:torch.Tensor=None,
                Dy_distill:torch.Tensor=None,
                mode:str='Derivative'
        ) -> torch.Tensor:
        ...
        if x_distill is not None:
            u_distill = self.forward(x_distill)
            Du_distill = vmap(jacrev(self.forward_single))(x_distill)[:,:,:]
            
            if mode == 'Output':
                distillation_loss = self.loss_container(u_distill, y_distill)
            elif mode == 'Derivative':
                distillation_loss = self.loss_container(Du_distill, Dy_distill)
            elif mode == 'Sobolev':
                distillation_loss = self.loss_container(u_distill, y_distill)
                distillation_loss+= self.loss_container(Du_distill, Dy_distill)
            elif mode == 'Forgetting':
                distillation_loss = torch.zeros_like(bc_loss)
            elif mode == 'PINN':
                uXXX_distill = vmap(jacrev(jacrev(jacrev(self.forward_single))))(x_distill)[:,0,:,:,:]
                ut_distill = Du_distill[:,0,0]
                ux_distill = Du_distill[:,0,1]
                uxxx_distill = uXXX_distill[:,1,1,1]
                pde_pred_distill = ut_distill + u_distill.reshape((-1))*ux_distill + nu*uxxx_distill
                distillation_loss = self.loss_container(pde_pred_distill, torch.zeros_like(pde_pred_distill))
        else:
            distillation_loss = torch.zeros_like(bc_loss)
        
        loss = self.pde_weight*pde_loss + self.init_weight*init_loss + self.bc_weight*bc_loss + self.sys_weight*out_loss + self.distillation_weight*distillation_loss
        if print_to_screen:
            print(f'Step: {step}, tot loss: {loss}, out loss: {out_loss}, PDE loss: {pde_loss}, boundary loss: {bc_loss}, initial loss: {init_loss}, distillation loss: {distillation_loss} ')
        return step, out_loss, pde_loss, init_loss, bc_loss, distillation_loss, loss
    ...
    def student_loss_fn(self,
        ...
        if mode == 'Derivative':
            ...
            sys_loss = torch.zeros_like(u_pde).mean()
        else:
