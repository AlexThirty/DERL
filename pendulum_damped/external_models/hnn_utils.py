...
def integrate_model(model, t_span, y0, b=0., l=10., m=1., fun=None, **kwargs):
  def default_fun(t, np_x):
      x = torch.tensor( np_x, requires_grad=True, dtype=torch.float32, device=model.device)
      x = x.view(1, np.size(np_x)) # batch size of 1
      dx = model.time_derivative(x).data.cpu().numpy().reshape(-1)
      dx[1] -= b * np_x[1]
      return dx
