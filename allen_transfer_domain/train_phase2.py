...
train_dataset = torch.load(os.path.join('data', 'dataset_rand_up.pth'), weights_only=False)
distill_dataset = torch.load(os.path.join('data', f'dataset_distillation_{name}_{mode}_phase1.pth'), weights_only=False)
eval_dataset = torch.load(os.path.join('data', 'dataset_full.pth'), weights_only=False)
bc_dataset = torch.load(os.path.join('data', f'boundary_full.pth'), weights_only=False)
#else:
#    bc_dataset = None
...
torch.save(model.state_dict(), f'saved_models/pinn_{name}_{mode}_phase2')
# %%
model.load_state_dict(torch.load(f'saved_models/pinn_{name}_{mode}_phase2'))
...
if not os.path.exists(f'{name}/plots_{mode}_phase2'):
    os.mkdir(f'{name}/plots_{mode}_phase2')
...
plt.savefig(f'{name}/plots_{mode}_phase2/pred_solution.png')
plt.close()
...
plt.savefig(f'{name}/plots_{mode}_phase2/error.png')
plt.close()
...
with open(f'{name}/plots_{mode}_phase2/traindata.npy', 'wb') as f:
...
plt.ylabel('Loss')
plt.savefig(f'{name}/plots_{mode}_phase2/train_losses.png')
plt.close()
...
with open(f'{name}/plots_{mode}_phase2/testdata.npy', 'wb') as f:
...
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.savefig(f'{name}/plots_{mode}_phase2/test_losses.png')