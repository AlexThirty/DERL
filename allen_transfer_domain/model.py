...
class AllenNet(torch.nn.Module):
    ...
    def loss_fn_phase2(self,
        ...
        # lambda*(uxx + uyy) - u + u^3 = 0
        # Calculate the pde_residual
        pde_pred = lam*(Hy_new_pred[:,0,0] + Hy_new_pred[:,1,1]) - y_new_pred.reshape((-1)) + y_new_pred.reshape((-1))**3
        # Calculate the loss
        pde_loss = self.loss_container(pde_pred, allen_cahn_forcing(x_new))
        ...
        if mode == 'PINN':
            ...
            spec_loss = torch.tensor([0.], device=self.device)
    ...
    def eval_losses_phase2(self, step:int,
        ...
        if print_to_screen:
            print(f'Step: {step}, total loss: {tot_loss}')
            print(f'pde loss: {pde_loss}, out loss {out_loss}, Dout loss: {der_loss}, bc loss: {bc_loss}, hes loss: {hes_loss}')
