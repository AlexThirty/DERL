...
parser.add_argument('--sys_weight', default=5., type=float, help='Weight for the rho loss')
...
parser.add_argument('--mode', default='Forgetting', type=str, help='Mode: -1 for PINN learning, 0 for derivative learning, 1 for output learning')
...
train_dataset = torch.load(os.path.join('data', 'dataset_rand_down_right.pth'), weights_only=False)
distill_dataset = torch.load(os.path.join('data', 'dataset_distillation_phase0.pth'), weights_only=False)
eval_dataset = torch.load(os.path.join('data', 'dataset_down.pth'), weights_only=False)
bc_dataset = torch.load(os.path.join('data', f'boundary_down.pth'), weights_only=False)
#else:
#    bc_dataset = None
...
def train_loop(epochs:int,
    ...
    for epoch in range(epochs):
        ...
        for step, (train_data, distill_data, bc_data) in enumerate(zip(train_dataloader, cycle(distill_dataloader), cycle(boundary_dataloader))):
            if step > train_steps:
                break
            ...
            Dy_distill = distill_data[2].to(device).float()[:,0]
            D2y_distill = distill_data[3].to(device).float()[:,0]
        ...
        with torch.no_grad():
            for (test_data, bc_data) in zip(eval_dataloader, cycle(bc_dataloader)):
...
train_loop(epochs=epochs, train_dataloader=train_dataloader, distill_dataloader=distill_dataloader, eval_dataloader=eval_dataloader, boundary_dataloader=bc_dataloader, print_every=1)
...
torch.save(model.state_dict(), f'saved_models/pinn_{name}_{mode}_phase1')
# %%
model.load_state_dict(torch.load(f'saved_models/pinn_{name}_{mode}_phase1'))
...
if not os.path.exists(f'{name}/plots_{mode}_phase1'):
    os.mkdir(f'{name}/plots_{mode}_phase1')
...
fig, ax = plt.subplots()
contour = ax.contourf(x_pts, y_pts, u_pred, levels=50, cmap=cm.jet)
ax.set_xlabel('x')
ax.set_ylabel('y')
fig.colorbar(contour, ax=ax, orientation='vertical')
fig.suptitle('Predicted solution')
plt.savefig(f'{name}/plots_{mode}_phase1/pred_solution.png')
plt.close()

# Plot the error wrt the true solution
fig, ax = plt.subplots()
contour = ax.contourf(x_pts, y_pts, u_err, levels=50, cmap=cm.jet)
ax.set_xlabel('x')
ax.set_ylabel('y')
fig.colorbar(contour, ax=ax, orientation='vertical')
fig.suptitle('Error of the predicted solution')
plt.savefig(f'{name}/plots_{mode}_phase1/error.png')
plt.close()
...
with open(f'{name}/plots_{mode}_phase1/traindata.npy', 'wb') as f:
...
plt.ylabel('Loss')
plt.savefig(f'{name}/plots_{mode}_phase1/train_losses.png')
plt.close()
...
with open(f'{name}/plots_{mode}_phase1/testdata.npy', 'wb') as f:
...
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.savefig(f'{name}/plots_{mode}_phase1/test_losses.png')
...
output_dataset = torch.load(os.path.join('data', 'dataset_rand_down.pth'), weights_only=False)

#else:
#    bc_dataset = None
# Generate the dataloader
output_dataloader = DataLoader(output_dataset, batch_size, generator=gen, shuffle=True, num_workers=4)
...
for train_data in output_dataloader:
    x = train_data[0]
    pde_x.append(x)
...
torch.save(pde_distillation_dataset, os.path.join('data', f'dataset_distillation_{name}_{mode}_phase1.pth'))
