...
pts_rand = random_pts
def finite_difference_pdv(x: np.array, h=1e-5):
    pdv = np.zeros_like(x)
    for i in range(x.shape[1]):
        ...
        x_forward[:, i] += h
        x_backward[:, i] -= h
        pdv[:, i] = (allen_cahn_true(x_forward) - allen_cahn_true(x_backward)) / (2 * h)
    return pdv

pdv_rand_fd = finite_difference_pdv(random_pts)
...
for h in step_sizes:
    pdv_rand_fd = finite_difference_pdv(random_pts, h)
    pdv_rand_fd = torch.tensor(pdv_rand_fd, dtype=torch.float32)
    rand_dataset_h = TensorDataset(torch.tensor(pts_rand, dtype=torch.float32), torch.tensor(u_rand, dtype=torch.float32), pdv_rand_fd, torch.tensor(hes_rand, dtype=torch.float32))
    torch.save(rand_dataset_h, f'data/rand_dataset_{h}.pth')
