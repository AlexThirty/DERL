...
parser.add_argument('--name', default='multi', type=str, help='Experiment name')
...
from tuner_multi_results import tuner_results
...
train_dataset = torch.load('train_data/train_data_multi_rand.pt')
true_dataset = torch.load('val_data/val_data_multi_rand.pt')
boundary_dataset = torch.load('boundary_dataset_multi.pt')
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
bc_loader = DataLoader(boundary_dataset, batch_size=batch_size, shuffle=True)
true_loader = DataLoader(train_dataset, batch_size=10000, shuffle=True)

print('Data loaded!')

print(true_dataset[:][0].shape)

activation = torch.nn.Tanh()

model = AllenNet(in_dim=4,
    bc_weight=bc_weight,
...
torch.save(model.state_dict(), f'{name}/saved_models/allen_multi_net{title_mode}')
# %%
model.load_state_dict(torch.load(f'{name}/saved_models/allen_multi_net{title_mode}'))




if not os.path.exists(f'{name}/plots_multi{title_mode}'):
    os.mkdir(f'{name}/plots_multi{title_mode}')
...
with open(f'{name}/plots_multi{title_mode}/traindata.npy', 'wb') as f:
...
plt.ylabel('Loss')
plt.savefig(f'{name}/plots_multi{title_mode}/train_losses.png')
plt.close()
...
with open(f'{name}/plots_multi{title_mode}/valdata.npy', 'wb') as f:
...
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.savefig(f'{name}/plots_multi{title_mode}/val_losses.png')