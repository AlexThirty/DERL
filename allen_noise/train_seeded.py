...
parser.add_argument('--noise', default=30, type=float, help='Seed for the experiment')
...
title_mode = mode + f'_noise_{args.noise}'
...
train_dataset = test_dataset = torch.load(os.path.join('data', f'{prefix}_dataset.pth'), weights_only=False)
bc_dataset = torch.load(os.path.join('data', f'bc_dataset.pth'), weights_only=False)
#else:
from torch.utils.data import TensorDataset
ders = train_dataset[:][2]
print(train_dataset[:][0].shape)
ders_noise = ders + torch.randn_like(ders) * args.noise

vals_noise = train_dataset[:][1] + torch.randn_like(train_dataset[:][1]) * args.noise
train_dataset = TensorDataset(train_dataset[:][0], vals_noise, ders_noise, train_dataset[:][3])
#    test_dataset = TensorDataset(test_dataset[:][0], test_dataset[:][1], test_dataset[:][2], test_dataset[:][3])
#train_dataset = TensorDataset(train_dataset[:][0], train_dataset[:][1], ders_noise, train_dataset[:][3])
#    bc_dataset = None
# Generate the dataloader
train_dataloader = DataLoader(train_dataset, batch_size, generator=gen, num_workers=4, shuffle=True)
test_dataloader = DataLoader(test_dataset, 10000, generator=gen, num_workers=4, shuffle=True)
bc_dataloader = DataLoader(bc_dataset, batch_size, generator=gen, num_workers=4, shuffle=True)
...
def train_loop(epochs:int,
    ...
    for epoch in range(epochs):
        ...
        for step, (train_data, bc_data) in enumerate(zip(train_dataloader, cycle(bc_dataloader))):
            ...
            #Dy_train += torch.randn_like(Dy_train) * args.noise
